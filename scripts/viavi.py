...
def establecer_RSRP_RSRQ(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df[cols_potencia] = df[cols_potencia].apply(
        pd.to_numeric, errors="coerce"
    )
    
    df[cols_calidad] = df[cols_calidad].apply(
        pd.to_numeric, errors="coerce"
    )
...
def establecer_RSRP_RSRQ(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Force all entries to numeric; invalid strings (e.g., "--", "LTE ...") become NaN
    df[cols_potencia] = df[cols_potencia].apply(pd.to_numeric, errors="coerce")
    df[cols_calidad] = df[cols_calidad].apply(pd.to_numeric, errors="coerce")

    # Compute maxima safely (now columns are guaranteed numeric)
    df["RSRP (dBm)"] = df[cols_potencia].max(axis=1)
    df["RSRQ (dB)"] = df[cols_calidad].max(axis=1)
...
def generar_mapa_rsrp(
    ...
    plt.scatter([lon0], [lat0], color="fuchsia", marker="o")
    plt.scatter([lon1], [lat1], color="orange", marker="^")
...
def main():
    ...
    df_unidos["GlobalCellId"] = np.nan
    df_unidos["PCI"] = df_unidos["PCI"].replace("--", np.nan)
    df_unidos["Sector"] = df_unidos["Sector"].replace("--", np.nan)
    df_unidos["LTE NB-RSRP_Top1_byPower [dBm]"] = df_unidos["LTE NB-RSRP_Top1_byPower [dBm]"].replace("--", np.nan)
    df_unidos["LTE NB-RSRQ_Top1_byPower [dB]"] = df_unidos["LTE NB-RSRQ_Top1_byPower [dB]"].replace("--", np.nan)
